...
class qikan_cqvip_com:
    ...
    def get_browser_cookies_(self):
        ...
        #browser.set.load_mode.eager()  # 设置为eager模式
    ...
    def analysis_get_Article_Detail(self,response,article_url):
        ...
        for div in subject_divs:
            ...
            # 中文关键词：提取包含<a>标签的<span>（排除label span）
            cn_terms = []
            for child in div.children:
                name = getattr(child, "name", None)
                # 遇到<em>标签就停止，说明中文关键词已经结束
                if name == "em":
                    break
                # 只处理span标签，且排除class为"label"的span
                if name == "span":
                    # 检查是否是label span
                    span_classes = child.get("class", [])
                    if "label" in span_classes:
                        continue
                    # 提取span内的<a>标签文本
                    a = child.find("a")
                    if a:
    ...
    def main(self):
        self.csv_all_path = os.path.join(self.csv_dir, "空天技术.csv")

        self.url = 'https://qikan.cqvip.com/Qikan/Journal/Summary?kind=1&gch=91348X&from=Qikan_Search_Index'
        
        # 获取浏览器
        self.browser = self.get_browser_cookies()

        gch = self.url.split("&gch=")[-1].split("&")[0]
        cluster_items_list = self.get_cluster_items(self.url)
